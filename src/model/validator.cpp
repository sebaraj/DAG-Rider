...
Validator::Validator(const std::string &keypair, uint16_t port, uint16_t tx_port,
    ...
    tx_address_
        = boost::asio::ip::tcp::endpoint(boost::asio::ip::address::from_string("0.0.0.0"), tx_port);
    block_address_ = boost::asio::ip::tcp::endpoint(
        boost::asio::ip::address::from_string("0.0.0.0"), block_port);
}

void Validator::create_keypair(const std::string &kps, ed25519_public_key &public_key,
                               ed25519_secret_key &secret_key) {
...
NodePublicKey Validator::create_public_key_from(const ed25519_secret_key &secret_key) {
...
void to_json(nlohmann::json &j, const Validator &validator) {
    ...
    j = nlohmann::json{{"address", validator.address_.address().to_string() + ":"
...
void from_json(const nlohmann::json &j, Validator &validator) {
    ...
    validator.address_ = boost::asio::ip::tcp::endpoint(
        boost::asio::ip::make_address(address_str.substr(0, address_str.find(":"))),
        std::stoi(address_str.substr(address_str.find(":") + 1)));
    validator.tx_address_ = boost::asio::ip::tcp::endpoint(
        boost::asio::ip::make_address(tx_address_str.substr(0, tx_address_str.find(":"))),
        std::stoi(tx_address_str.substr(tx_address_str.find(":") + 1)));
    validator.block_address_ = boost::asio::ip::tcp::endpoint(
        boost::asio::ip::make_address(block_address_str.substr(0, block_address_str.find(":"))),
