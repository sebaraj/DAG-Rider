...
std::string NetworkErrorToString(NetworkError error, const std::string &address, uint16_t retry) {
    std::stringstream ss;
    switch (error) {
...
void LogNetworkError(NetworkError error, const std::string &address, uint16_t retry) { std::cerr << NetworkErrorToString(error, address, retry) << std::endl; }
