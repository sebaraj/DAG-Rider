...
public:
  ...
  net::awaitable<void> dispatch(Writer &writer, const std::string &message) {
    ...
    co_await net::async_write(writer, net::buffer(response),
                              net::use_awaitable);
    co_return;
  };
};

template <typename Handler> class Receiver {
public:
  Receiver(const std::string &address, unsigned short port,
...
Receiver<Handler>::Receiver(const std::string &address, unsigned short port,
                            std::shared_ptr<Handler> handler)
    : handler_(handler), endpoint_(net::ip::make_address(address), port),
      io_context_() {}

template <typename Handler> void Receiver<Handler>::start() {
  net::co_spawn(io_context_, run(), net::detached);
  io_context_.run();
}

template <typename Handler> net::awaitable<void> Receiver<Handler>::run() {
  tcp::acceptor acceptor(io_context_, endpoint_);
  std::cout << "Listening on " << endpoint_.address().to_string() << ":"
            << endpoint_.port() << std::endl;
  for (;;) {
    tcp::socket socket = co_await acceptor.async_accept(net::use_awaitable);
    net::co_spawn(io_context_, handle_connection(std::move(socket)),
                  net::detached);
  }
}
...
net::awaitable<void> Receiver<Handler>::handle_connection(tcp::socket socket) {
  ...
  for (;;) {
    try {
      ...
      auto bytes_transferred = co_await net::async_read_until(
          socket, net::dynamic_buffer(data), "\n", net::use_awaitable);
